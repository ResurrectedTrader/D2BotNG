/**
 * D2 area/level id (Levels.txt Id = identity.area) -> name.
 *
 * Hand-generated, unlike the skill, monster and class names, which `gameNames.ts` reads from the
 * game's own tables at runtime. It has to be: `d2itemtoolkit` does not ship Levels.txt, so there
 * is nothing to look an area id up in. That also means these names do not follow the game's locale
 * — they are English until the table arrives.
 *
 * Generated by scripts/gen-names.js from Levels.txt + the ENG string tables (v96):
 * LevelName resolved against the string tables, falling back to kolbot's areas.txt.
 */
